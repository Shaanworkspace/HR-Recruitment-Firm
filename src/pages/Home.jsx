...
function LogoImg({ logo, name, dim = "w-9 h-9" }) {
  return <img src={logo} alt={name} className={`${dim} object-contain shrink-0`} />
}

function LogoCard({ name, logo }) {
  return (
    <div className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl px-5 py-3 min-w-[180px] shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
...
function StatCard({ number, label }) {
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-lg hover:-translate-y-1 transition">
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden bg-slate-900 -mt-20">
        <img src="/hero-bg.jpg" alt="" className="absolute inset-0 w-full h-full object-cover opacity-25" />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-900 via-slate-900/85 to-blue-900/70" />
        <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-blue-500/20 blur-3xl animate-blob pointer-events-none" />
        <div className="absolute bottom-0 -left-24 w-96 h-96 rounded-full bg-purple-500/20 blur-3xl animate-blob pointer-events-none" style={{ animationDelay: '3s' }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-20 md:pt-40 md:pb-28">
          ...
          <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { number: "700+", label: "Sales Professionals Placed" },
              { number: "12,000", label: "Growing Talent Network" },
              { number: "150+", label: "Companies Hiring" },
              { number: "40", label: "Average Candidate Shortlisting" },
            ].map((s, i) => (
      ...
      <section className="py-12 border-y border-gray-100 bg-gray-50/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal>
            <p className="text-center text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Trusted by Growing Businesses</p>
            <p className="text-center text-gray-600 text-sm mb-8">Build your career at companies that are shaping modern B2B sales.</p>
          </Reveal>
          <LogoMarquee />
        </div>
      </section>
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Reveal>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Build Your Career at Companies That Are Shaping Modern B2B Sales</h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto mb-10">Whether your dream is joining a fast-growing startup or a global SaaS company, we're here to help you take the next step.</p>
          </Reveal>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {logoCompanies.map((c, i) => (
              <Reveal key={c.name} delay={(i % 5) * 60}>
                <div className="bg-white border border-gray-200 rounded-xl px-4 py-6 text-center hover:border-blue-300 hover:shadow-md hover:-translate-y-1 transition group">
                  <div className="w-14 h-14 mx-auto mb-3 flex items-center justify-center">
                    <LogoImg logo={c.logo} name={c.name} dim="w-12 h-12" />
                  </div>
                  <span className="text-sm font-semibold text-gray-700 group-hover:text-blue-600 transition">{c.name}</span>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="py-16 md:py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: "Sales Hiring", items: ["Sales Development Representatives", "Business Development Representatives", "Inside Sales Executives", "Account Executives", "Sales Managers"] },
              { title: "Customer Success Hiring", items: ["Customer Success Executives", "Relationship Managers", "Customer Success Managers", "Account Managers"] },
              { title: "Revenue Operations", items: ["Revenue Operations", "Sales Operations", "CRM Specialists", "Business Analysts"] },
              { title: "Leadership Hiring", items: ["Business Heads", "Sales Heads", "Growth Managers", "Revenue Leaders"] },
            ].map((cat, i) => (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <Reveal key={item.title} delay={(i % 3) * 80}>
                <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 hover:shadow-md hover:-translate-y-1 transition h-full">
            ...
            {[
              ...
              <Reveal key={step.num} delay={i * 90} className="flex-1">
                <div className="flex flex-col items-center text-center relative">
                  <div className="w-14 h-14 rounded-full bg-blue-600 text-white font-bold text-lg flex items-center justify-center mb-3 shadow-lg shadow-blue-600/30">{step.num}</div>
                  <h3 className="font-bold text-gray-900 mb-1">{step.title}</h3>
                  <p className="text-sm text-gray-600 max-w-[200px]">{step.desc}</p>
                  {i < 4 && <div className="hidden md:block absolute top-7 left-[60%] w-full h-0.5 border-t-2 border-dashed border-blue-300" />}
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-10">
            {careerRoles.map((role, i) => (
              <Reveal key={role} delay={(i % 6) * 50} className="inline-block">
                <span className="bg-white border border-gray-200 rounded-full px-5 py-2.5 text-sm font-medium text-gray-700 hover:border-blue-300 hover:text-blue-600 hover:-translate-y-0.5 transition cursor-default inline-block">
                  {role}
                </span>
              </Reveal>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal>
            <div className="max-w-2xl">
              ...
              <p className="text-gray-600 mb-10">From early-stage SaaS to enterprise fintech, we hire revenue talent across the sectors shaping India's B2B landscape.</p>
            </div>
          </Reveal>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {industries.map((ind, i) => (
              <Reveal key={ind} delay={(i % 5) * 60}>
                <div className="group bg-white border border-gray-200 rounded-2xl p-6 hover:border-blue-300 hover:shadow-md hover:-translate-y-1 transition h-full">
                  <div className="w-10 h-10 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-sm mb-4 group-hover:bg-blue-600 group-hover:text-white transition">{String(i + 1).padStart(2, '0')}</div>
      ...
      <section className="relative py-16 md:py-24 overflow-hidden bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Reveal>
            <div className="max-w-3xl">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Building Careers. Powering Revenue Teams.</h2>
      ...
      <section className="py-16 md:py-24 bg-gray-900 text-white relative overflow-hidden">
        <div className="absolute -top-20 -right-20 w-80 h-80 rounded-full bg-blue-500/20 blur-3xl animate-blob pointer-events-none" />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <Reveal key={i} delay={i * 100}>
                <div className="bg-white border border-gray-200 rounded-2xl p-6 text-left hover:shadow-md hover:-translate-y-1 transition h-full">
      ...
      <section className="py-16 md:py-24 bg-dots">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-8">
            {["Interview Tips", "Resume Guides", "Sales Career Advice", "Hiring Insights", "LinkedIn Optimization", "Industry Trends"].map((t, i) => (
              <Reveal key={t} delay={(i % 6) * 50} className="inline-block">
                <span className="bg-gray-50 border border-gray-200 rounded-full px-5 py-2.5 text-sm font-medium text-gray-700 hover:border-blue-300 hover:text-blue-600 hover:-translate-y-0.5 transition cursor-default inline-block">{t}</span>
              </Reveal>
            ))}
          </div>
          <Reveal delay={120}>
      ...
      {/* Final CTA */}
      <section className="py-16 md:py-24 bg-gradient-to-br from-purple-600 to-blue-700 text-white relative overflow-hidden">
        <div className="absolute -bottom-24 -left-24 w-96 h-96 rounded-full bg-white/10 blur-3xl animate-blob pointer-events-none" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <Reveal>
